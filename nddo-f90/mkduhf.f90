...
! 3pz
      elseif(nq.eq.3.and.nl.eq.4)then
       amplit(icount)=.206012907d0*dsqrt(ei*ei*ei*ei*ei*ei*ei)*exp(eir)*r*z_relative
         ...
         rhora=rhora+amplit(icount)*amplit(icount)*adens(icount,icount)
         rhorb=rhorb+amplit(icount)*amplit(icount)*bdens(icount,icount)
      end do
      end do
    do k=1,numat
       if(nbas(species(k)).gt.1)then 
          ...
          rhora=rhora+amplit(i)*amplit(j)*adens(j,i)*two
          rhorb=rhorb+amplit(i)*amplit(j)*bdens(j,i)*two
