...
,xpoint(194),ypoint(194),zpoint(194)
...
print*,'-----------------------------------------------'
print*,''
write(*,*)'Density functionals will be evaluated using restricted HF density'
call cpusec(time1)
...
xpoint=dsin(thleb)*dcos(pleb)
ypoint=dsin(thleb)*dsin(pleb)
zpoint=dcos(thleb)
...
! density gradient at point
       call mkd(rhorp,xleb+delta,yleb,zleb)
       call mkd(rhorm,xleb-delta,yleb,zleb)
       dx=rhorp-rhorm
       call mkd(rhorp,xleb,yleb+delta,zleb)
       call mkd(rhorm,xleb,yleb-delta,zleb)
       dy=rhorp-rhorm
       call mkd(rhorp,xleb,yleb,zleb+delta)
       call mkd(rhorm,xleb,yleb,zleb-delta)
       dz=rhorp-rhorm
       dx=dx/2.0d0/delta
       dy=dy/2.0d0/delta
       dz=dz/2.0d0/delta
       g1=dx**2+dy**2+dz**2
         call crlyp(rhor,g1,fout,dfdra,dfdgaa,dfdgab)

         clyp=clyp+(weight(irad)*r*r*two*a*fout/(one-point(irad))**2) &
             *wleb(jleb)*4.0d0*pi*wn(iatom)

        call xcrb3lyp(rhor,g1,fout,dfdra,dfdgaa,dfdgab)

        b3lyp=b3lyp+(weight(irad)*r*r*two*a*fout/(one-point(irad))**2) &
             *wleb(jleb)*4.0d0*pi*wn(iatom)
       
        call c_rks_vwn5(rhor,fout,dfdra)

         vwn=vwn+(weight(irad)*r*r*two*a*fout/(one-point(irad))**2) &
              *wleb(jleb)*4.0d0*pi*wn(iatom)
       
         call x_rks_pbe(rhor,g1,fout,dfdra,dfdgaa)

       pbe=pbe+(weight(irad)*r*r*two*a*fout/(one-point(irad))**2) &
             *wleb(jleb)*4.0d0*pi*wn(iatom)


        call xrb88(rhor,g1,fout,dfdra,dfdgaa)
...
! get exact exchange 
call hfexc(coulomb,exchange,ekinetic)
!open(unit=1,file='exchange')
!read(1,*)exchange
!close(1)

print*,'Radial Integration using 48 point Gauss-Legendre grid'
print*,'Angular Integration using 194 point Lebedev-Laikov grid'      

print*,'**************************************************************'
print*,'|Density integrates to ',sumx,' electrons using current grid '
print*,'|Actual electron count =',float(nelectrons)
print*,'|Approximate Error in evaluation of functionals = ',sumx-float(nelectrons)
print*,'**************************************************************'

write(*,*)''
       PRINT*,'SUMMARY OF EXCHANGE AND/OR CORRELATION ENERGIES:'
       PRINT*,'X = EXCHANGE ONLY    C = CORRELATION ONLY  XC = COMBINATION'
       print*,'All values are in eV!'
       print*,'-----------------------------------'
       print*,'SCF Total Energy =',ekinetic+coulomb+exchange+enuc
       print*,'SCF One Particle Energy =',ekinetic
       print*,'SCF Coulomb  E(J) = ',coulomb
       print*,'SCF Exchange E(X) = ',exchange    
       print*,'LDA (Xalpha) E(X) = ',xalp*constlda*autoev
       print*,'PBE          E(X) = ',pbe*autoev
       print*,'BECKE88      E(X) = ',b88*autoev
       print*,'LYP          E(C) = ',clyp*autoev
       print*,'VWN          E(C) = ',vwn*autoev
       print*,'B3LYP        E(XC)= ',b3lyp*autoev+0.2d0*exchange
